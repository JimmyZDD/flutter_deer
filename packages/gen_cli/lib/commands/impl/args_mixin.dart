...
mixin ArgsMixin {
  final List<String> _args = GenCli.arguments;
...
List<String> _getArgs() {
  var args = List.of(GenCli.arguments);
  _removeDefaultArgs(args);
...
int _getIndexArg(String arg) {
  return GenCli.arguments.indexWhere((element) => element == arg);
}

String _getArg(String arg) {
  var index = _getIndexArg(arg);
  if (index != -1) {
    if (index + 1 < GenCli.arguments.length) {
      index++;
      return GenCli.arguments[index];
    } else {
